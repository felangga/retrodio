...
bool handleKeyboardTouch(lgfx::LGFX_Device& lcd, MacComponent* keyboardComponent, MacComponent* inputComponent, int touchX, int touchY) {
  if (!keyboardComponent || keyboardComponent->type != COMPONENT_KEYBOARD) return false;
  if (!inputComponent || inputComponent->type != COMPONENT_INPUT_FIELD) return false;
  ...
  if (touchX < x || touchX > x + w || touchY < y || touchY > y + h) return false;
